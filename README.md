...
4. Подготовьте данные:
   ...
   - (Опционально) Для ускорения работы распакуйте архив `data_clean_text.7z` и скопируйте предоставленный файл `data_clean_text.csv` в рабочую директорию. При его наличии код автоматически загрузит очищенный текст, минуя длительный этап предобработки.
...
│   ├── toxic_comments.csv              # Исходные данные (нужно распаковать из архива)
│   └── data_clean_text.csv             # Очищенный текст (опционально, для ускорения, но нужно распаковать из архива)
